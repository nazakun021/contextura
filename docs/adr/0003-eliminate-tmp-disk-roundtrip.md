...
**Phase 2 (future):** Explore passing raw pixel data directly to `vision-helper` via stdin pipe or shared memory:
- Option A: Pipe raw RGBA bytes + dimensions header via stdin. `vision-helper` creates a `CGImage` from raw data instead of reading a PNG.
...
Keep the `/tmp/contextura-frame-latest.png` debug copy as an opt-in debug feature (e.g., behind a `--debug-frames` flag), not as a pipeline dependency.

## Consequences

- **Positive (Phase 1):** Eliminates the shared `/tmp` security concern.
- **Positive (Phase 2):** Reduces per-frame latency and removes disk I/O from the hot path.
