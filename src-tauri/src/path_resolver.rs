...
impl AppConfig {
    pub fn new(bundle_id: String, process_id: i32, name_hint: String, path_resolver: Arc<PathResolver>) -> Self {
        Self {
            bundle_id,
            process_id,
            name_hint,
...
impl PathResolver {
    ...
    pub fn models_dir(&self, app_handle: Option<&tauri::AppHandle>) -> anyhow::Result<PathBuf> {
        if self.is_headless {
            let root = self.custom_root.clone().unwrap_or_else(|| {
        ...
        let mut path = dirs::data_local_dir()
            .ok_or_else(|| anyhow::anyhow!("No data local dir available"))?;
    ...
    pub fn resolve_binary(&self, binary_name: &str, app_handle: Option<&tauri::AppHandle>) -> anyhow::Result<PathBuf> {
        ...
        candidates.push(manifest_dir.join("binaries").join(format!("{binary_name}-aarch64-apple-darwin")));
